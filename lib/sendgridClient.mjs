...
// unit-tested without a real network call lives in lib/contact.mjs; this
// file is deliberately the only place that touches `fetch` and the key, so
// tests can inject a fake `httpClient` and never send real mail. Generic on
// purpose — the exported names below (NotifyConfigError/NotifySendError)
// are kept as-is: this module isn't specific to the contact route, and the
// name isn't claimed by anything else.
//
